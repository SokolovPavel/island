...
#pragma target 3.0
#include "SuimonoFunctions.cginc"
#pragma surface surf SuimonoRefract vertex:vertexSuimonoDisplace noambient
#pragma glsl
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	float4 uv1 = uvs;
	float4 uv2 = uvs;//IN.screenPos;
	float4 uvx = uvs;//IN.screenPos;
	...
	half depth1 = tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(IN.screenPos)).r;
	half rShift = _RefrShift * (lerp(0.0,2.0,useNormal.y) * _RefrStrength * edgeSpread.a);
	depth1 += tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(float4(uvx.x+rShift, uvx.y, uvx.z,uvx.w))).r;
	depth1 += tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(float4(uvx.x-rShift, uvx.y, uvx.z,uvx.w))).b;
	...
	half depth2 = tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(float4(uv2.x, uv2.y, uv2.z,uv2.w))).r;
	depth2 += tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(float4(uv2.x+rShift, uv2.y, uv2.z,uv2.w))).r;
	depth2 += tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(float4(uv2.x-rShift, uv2.y, uv2.z,uv2.w))).b;
